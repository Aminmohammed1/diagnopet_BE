...
async def get_by_list_of_ids(
    ...
    result = await db.execute(select(Test).where(Test.id.in_(ids)))
...
async def get_tests_by_booking_id(db: AsyncSession, booking_id: int) -> List[Test]:
    result = await db.execute(
        select(Test)
        .join(BookingItem, BookingItem.test_id == Test.id)
